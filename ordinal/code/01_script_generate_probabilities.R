...
user_group1 = list(pI31 = c(1/9, 3/9, 5/9),
                   ...
                   pI71 = c(1/28, 2/28, 3/28, 4/28, 5/28, 6/28, 7/28),  # modified from pI81 = c(1/36, 2/36, 3/36, 4/36, 5/36, 6/36, 7/36, 8/36)
...
user_group2 = list(pC31 = c(rep(1/3, 3)),
                   ...
                   pC53 = c(0.05, 0.2, 0.4, 0.25, 0.1),
                   pC71 = c(rep(1/7, 7)),  # modified from pC81 = c(rep(1/8, 8))
...
# Add information on which settings should be used in null case and which in effect/power case 
# (e.g., some settings would be duplicated for null case)
param_user = param_user %>% mutate(usefornull = usefornull1, useforpower = TRUE)
rm(user_group1, user_group2, usefornull1)

# Add settings from Funatogawa and Funatogawa, 2023
