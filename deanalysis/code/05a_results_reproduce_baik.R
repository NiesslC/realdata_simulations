...
fig2res = melt(performdat_degenes %>% filter(simul.data == "TCGA.KIRC"),
  ...
  mutate(nDE = factor(nDE, levels = c("pDE = 5%", "pDE = 10%", "pDE = 30%", "pDE = 60%")),
         Methods = factor(Methods, levels = c("edgeR", "edgeR.ql", "edgeR.rb",
                                              ...
                                              "voom.qn", "voom.sw", "ROTS", "BaySeq",
                                              "PoissonSeq", "SAMseq")))
# Check NAs in performance_measures (only affects trueFDR)
fig2res %>%
  group_by(Methods, performance_measure) %>%
  summarise(sumnan = sum(is.nan(performance_value))) %>%
  filter(sumnan > 0)
fig2res = fig2res %>% filter(!is.nan(performance_value))

# Make plot
pfig2_base = ggplot(fig2res, aes(x = Methods, y = performance_value, col = Methods)) +
  geom_boxplot() +
  scale_color_manual(values = unique(fig2res$Color)) +
  facet_grid(performance_measure ~ nDE, scales = "free") +
  labs(x = "", y = "") +
  facetted_pos_scales(y = list(
    performance_measure == "AUC" ~ scale_y_continuous(limits = c(0.5, 0.9), breaks = seq(0.5, 0.9, 0.1)),
