      ...
      for (disp.Type in disp.Types) {
        ...
        for (i in rep.start:rep.end) {
          for (s in nsample) {
            for (nde in nDE) {
              if (nde == 0) {
                SyntheticDataSimulation_new(
                  simul.data = simul.data,
                  dataset = paste(working.dir, datahead, "0DE_", s, "spc_rep_", i, ".rds",
                                  sep = ""),
                  fixedfold = fixedfold,
                  n.var = nvar,
                  samples.per.cond = s,
                ...
                SyntheticDataSimulation_new(
                  simul.data = simul.data,
                  dataset = paste(working.dir, datahead, nde, "DE_", s,
                                  "spc_fixedfold_upFrac_0.67_rep_", i, ".rds", sep = ""),
                  fixedfold = fixedfold,
                  n.var = nvar,
                  samples.per.cond = s,
                  n.diffexp = nde,
                ...
                for (frac in fraction.upregulated) {
                  SyntheticDataSimulation_new(
                    simul.data = simul.data,
                    dataset = paste(working.dir, datahead, nde, "DE_", s, "spc_upFrac_",
                                    frac, "_rep_", i, ".rds", sep = ""),
                    fixedfold = fixedfold,
                    n.var = nvar,
                    samples.per.cond = s,
                    n.diffexp = nde,
                    fraction.upregulated = frac,
                    dispType = disp.Type,
                    mode = mode,
                    RO.prop = RO.prop,
                    dataset.parameters = dataset.parameters,
...
SyntheticDataSimulation_new = function(simul.data,
                                       ...
                                       RO.prop = 5,
                                       dataset.parameters) {
  if (mode != "D" && mode != "S" && mode != "R" && mode != "OS" && mode != "DL") {
...
performance_plot_new = function(working.dir,
  ...
  type = switch(test.cond,
                D = "No Random outlier test / same dispersion ",
    ...
    for (s in nsample) {
      for (prop in fraction.upregulated) {
        for (tools in AnalysisMethods) {
          ...
          for (i in rep.start:rep.end) {
            if (fixedfold) {
              fileName = paste(working.dir, simul.data, "_", test.cond, "_", nde,
                               "DE_", s, "spc_fixedfold_upFrac_", prop,
                               "_rep_", i, "_", tools2, ".rds", sep = "")
            } else {
              fileName = paste(working.dir, simul.data, "_", test.cond, "_", nde,
            ...
            tpr_temp = append(tpr_temp,
                              length(which(FDR[indexTrue] < 0.1)) / length(FDR[indexTrue]))
            if (length(which(FDR < 0.1)) <= 5) {
              tfdr_temp = append(tfdr_temp, NaN)
            } else {
              tfdr_temp = append(tfdr_temp,
                                 length(setdiff(which(FDR < 0.1), indexTrue)) / length(which(FDR < 0.1)))
            }
